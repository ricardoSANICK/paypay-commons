 ...
 * or <a href=
 * "http://msdn.microsoft.com/en-us/library/ms679635%28v=vs.85%29.aspx">{@code sAMAccountName}</a>
 * (or a custom {@link #setSearchFilter(String) searchFilter}) in the form
	...
	private DirContext bindAsUser(String username, String password) {
		...
		try {
			...
			if ((rootException instanceof AuthenticationException)
					|| (rootException instanceof OperationNotSupportedException)) {
	...
	private void raiseExceptionForErrorCode(int code, NamingException exception) {
		...
		case PASSWORD_EXPIRED:
			throw new CredentialsExpiredException(messages.getMessage("LdapAuthenticationProvider.credentialsExpired",
					"User credentials have expired"), exception.getCause());
		...
		case ACCOUNT_LOCKED:
			throw new LockedException(
					...
					exception.getCause());
		default:
			throw badCredentials(exception.getCause());
		}
	}
